	...
	public function EnviaCorreo($dataMail)
	  {
			  ...
			  $mail->SMTPDebug    = 1;			   
				  ...
				  try {
						// $rowemail = explode(",",$arrayemail);
						// for($i=0;$i<=$count;$i++)
						// {
							  // echo "enviando..";
						$mail->AddAddress($dataMail["Address"],$dataMail["AddressUsr"]);
						// }
						$mail->SetFrom($dataMail["from"],$dataMail["fromUsr"]);    
						$mail->Subject = $dataMail["subject"];   
						$mail->MsgHTML($dataMail["mensaje"]);
						$mail->Send();
				  } 
